...
/*   Updated: 2021/01/22 20:07:47 by zminhas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_dot_flag(void *ptr, int i)
{
	...
	size = var.prec[1] < ft_hexlen_remix(adrs) ? ft_hexlen_remix(adrs) : var.prec[1];
	if (var.flag[0] == 1 || var.flag[0] == 3)
		while (i++ < var.prec[0] - size - 2)
			write(1, " ", 1);
	ft_putstr_fd("0x", 1);
	while (j++ < var.prec[1] - ft_hexlen_remix(adrs))
		write(1, "0", 1);
	ft_percent_x_remix(adrs);
	if (var.flag[0] == 2)
		while (i++ < var.prec[0] - size - 2)
			write(1, " ", 1);
	return (ft_hexlen_remix(adrs) + 2 + (--j) + (i ? --i : i));
}
...
int				ft_percent_p(void *ptr)
{
	...
	adrs = (unsigned long)ptr;
	if (var.flag[0] == 3)
		while (i++ < var.prec[0] - ft_hexlen_remix(adrs) - 2)
			write(1, " ", 1);
	ft_putstr_fd("0x", 1);
	...
	if (var.flag[0] == 2)
		while (i++ < var.prec[0] - ft_hexlen_remix(adrs) - 2)
			write(1, " ", 1);
	return (ft_hexlen_remix(adrs) + 2 + (i ? --i : i));
}
